...
function TranscriptionStep({ onStart, running, progress, message, error, fillers, onFillersChange }: { onStart: (overrides: TranscribeOverrides) => void; running: boolean; progress: number; message: string; error: string | null; fillers: boolean; onFillersChange: (value: boolean) => void }) {
  const [model, setModel] = useState("large-v3-turbo"); const [language, setLanguage] = useState("pt"); const [batch, setBatch] = useState(8); const [vad, setVad] = useState(true);
  return <section className="step-content enter"><div className="page-heading"><div><span className="eyebrow">02 / SPEECH ENGINE <i>転写</i></span><h1>Transcrição de alta precisão</h1><p>Word timing, waveform e VAD preparados para proteger cada fonema na edição.</p></div><span className="phase-number">02</span></div>
    <div className="two-col"><div className="panel glass"><div className="panel-title"><span><Icon name="wave"/></span><div><h3>Motor de reconhecimento</h3><p>Configuração aplicada somente ao iniciar</p></div></div>
      ...
      <div className="toggles"><Toggle checked={vad} onChange={setVad} label="Silero VAD · proteger regiões de voz"/><Toggle checked={fillers} onChange={onFillersChange} label="Marcar hesitações para revisão (visível na Curadoria)"/></div>
...
function BriefStep({ onAnalyze, running, progress, message, error }: { onAnalyze: (brief: SuggestionBrief) => void; running: boolean; progress: number; message: string; error: string | null }) {
  const [prompt, setPrompt] = useState("Encontre os melhores cortes para Instagram Reels e TikTok. Selecione trechos autocontidos que prendam a atenção nos primeiros segundos, desenvolvam uma ideia clara dentro da duração proposta e terminem com payoff forte. Priorize momentos interessantes, surpreendentes, emocionais, úteis ou provocativos que estimulem comentários, compartilhamentos e retenção, sem depender de contexto externo e sem distorcer o sentido da conversa."); const [count, setCount] = useState(15); const [min, setMin] = useState(40); const [max, setMax] = useState(90); const [tone, setTone] = useState("viral");
  return <section className="step-content enter"><div className="page-heading"><div><span className="eyebrow">03 / EDITORIAL INTELLIGENCE <i>解析</i></span><h1>Dê uma missão à inteligência</h1><p>Direcione o olhar editorial. A IA propõe; você continua como diretor.</p></div><span className="phase-number">03</span></div>
    <div className="brief-layout"><div className="panel glass"><div className="panel-title"><span><Icon name="brain"/></span><div><h3>Brief editorial</h3><p>Este texto será versionado junto ao projeto</p></div></div><textarea className="prompt-area" value={prompt} onChange={(e) => setPrompt(e.target.value)}/><div className="prompt-chips"><button onClick={() => setPrompt(prompt + " Priorize opiniões com contraste claro.")}>+ Contraste</button><button onClick={() => setPrompt(prompt + " Busque frases provocativas que convidem ao debate.")}>+ Hot takes</button><button onClick={() => setPrompt(prompt + " Preserve momentos de emoção genuína.")}>+ Emoção</button><button onClick={() => setPrompt(prompt + " Priorize histórias pessoais com começo, virada e conclusão.")}>+ Histórias</button></div><Field label="Direção"><div className="segmented"><button className={tone === "viral" ? "active" : ""} onClick={() => setTone("viral")}>Viral</button><button className={tone === "authority" ? "active" : ""} onClick={() => setTone("authority")}>Autoridade</button><button className={tone === "deep" ? "active" : ""} onClick={() => setTone("deep")}>Profundo</button></div></Field></div>
...
function EditPlanTimeline({ clip, plan }: { clip: SuggestedClip; plan: EditPlanDocument }) {
  ...
  const jlBoundaries = plan.segments.filter((segment) => segment.transition && (segment.transition.kind === "j_cut" || segment.transition.kind === "l_cut"));
  ...
  })}</div>{jlBoundaries.length > 0 && <div className="jl-boundaries" aria-label="Fronteiras J-cut/L-cut">{jlBoundaries.map((segment, index) => {
    const transition = segment.transition!;
    const kindLabel = transition.kind === "j_cut" ? "J-CUT" : "L-CUT";
    const sign = transition.audio_offset_seconds > 0 ? "+" : "";
    return <span key={`jl-${segment.timeline_order}-${index}`} className={`jl-chip ${transition.kind}`} title={`Áudio ${sign}${transition.audio_offset_seconds.toFixed(2)}s em relação ao corte de vídeo`}>{kindLabel} {sign}{transition.audio_offset_seconds.toFixed(2)}s</span>;
  })}</div>}<div className="edl-stats"><span><b>{plan.diagnostics.saved_seconds.toFixed(2)}s</b> removidos</span><span><b>{plan.diagnostics.cuts}</b> jump cuts</span><span><b>{Math.round(plan.diagnostics.crossfade * 1000)}ms</b> crossfade</span>{plan.jl_settings.enabled && <span><b>{plan.diagnostics.j_cuts + plan.diagnostics.l_cuts}</b> J/L-cuts</span>}</div></div>;
}
...
function FaceShotBadges({ faceIndex, sceneIndex, start, end }: { faceIndex: FaceIndexDocument | null; sceneIndex: SceneIndexDocument | null; start: number; end: number }) {
  if (!faceIndex || !sceneIndex?.scenes.length) return null;
  const scenesInRange = sceneIndex.scenes.filter((scene) => scene.end > start && scene.start < end);
  if (!scenesInRange.length) return null;
  return <div className="face-shot-badges" aria-label="Tipo de plano detectado por cena">{scenesInRange.map((scene) => {
    const summary = faceIndex.scenes.find((s) => s.scene_index === scene.index);
    const shotType = summary?.dominant_shot_type ?? "none";
    return <span key={scene.index} className={`shot-badge shot-${shotType}`} title={`Cena ${scene.index} · ${timestamp(scene.start)}–${timestamp(scene.end)} · ${summary?.sample_count ?? 0} amostra(s)`}>{shotTypeLabel(shotType)}</span>;
  })}</div>;
}

function shotTypeLabel(shotType: string): string {
  if (shotType === "close") return "Close";
  if (shotType === "two_shot") return "Dois planos";
  if (shotType === "wide") return "Plano aberto";
  return "Sem rosto";
}

const SCORE_LABELS: Record<string, string> = {
  spoken_hook: "Gancho falado", standalone_clarity: "Clareza autônoma", emotion: "Emoção",
  quotability: "Citabilidade", payoff: "Payoff", compression_safety: "Segurança de corte",
  audience_relevance: "Relevância",
};
...
function CurateStep({ clips, notes, provenance, transcript, analysis, showFillers, plans, planning, planProgress, planMessage, planError, onPlan, onNext, sceneIndex, sceneIndexArtifactId, detectingScenes, sceneProgress, sceneMessage, sceneError, onDetectScenes, faceIndex, detectingFaces, faceProgress, faceMessage, faceError, onDetectFaces, jlCutEnabled, onJlCutEnabledChange, onRegeneratePlan }: { clips: SuggestedClip[]; notes: string; provenance?: SuggestionSelection["provenance"]; transcript: TranscriptDocument | null; analysis: AnalysisDocument | null; showFillers: boolean; plans: Record<string, EditPlanDocument>; planning: boolean; planProgress: number; planMessage: string; planError: string | null; onPlan: (clips: SuggestedClip[]) => Promise<boolean>; onNext: (clips: SuggestedClip[]) => Promise<void>; sceneIndex: SceneIndexDocument | null; sceneIndexArtifactId: string | null; detectingScenes: boolean; sceneProgress: number; sceneMessage: string; sceneError: string | null; onDetectScenes: () => void; faceIndex: FaceIndexDocument | null; detectingFaces: boolean; faceProgress: number; faceMessage: string; faceError: string | null; onDetectFaces: () => void; jlCutEnabled: boolean; onJlCutEnabledChange: (value: boolean) => void; onRegeneratePlan: (clip: SuggestedClip) => void }) {
  ...
  const activeFillers = activeClip ? analysis?.fillers?.filter((filler) => filler.end >= activeClip.start_second && filler.start <= activeClip.end_second) ?? [] : [];
  const activeDensity = activeClip ? analysis?.speech_density?.filter((window) => window.end >= activeClip.start_second && window.start <= activeClip.end_second) ?? [] : [];
  const activeSpeechRatio = activeDensity.length ? activeDensity.reduce((sum, window) => sum + window.speech_ratio, 0) / activeDensity.length : null;
  const activePlan = activeClip ? plans[clipKey(activeClip)] : undefined;
  const selectedClips = clips.filter((_, index) => selected[index]);
  return <section className="step-content wide enter"><div className="page-heading"><div><span className="eyebrow">04 / HUMAN CURATION <i>選択</i></span><h1>A decisão final é sua</h1><p>Compare argumento, ritmo e fronteiras antes de levar um corte ao estúdio.</p></div><div className="page-heading-side"><ProvenanceBadge provenance={provenance}/><span className="result-count">{selected.filter(Boolean).length}<small>selecionados</small></span></div></div>
    {clips.length === 0 ? <div className="panel glass"><h3>Nenhum corte recomendado</h3><p>{notes}</p></div> : <div className="curate-layout"><div className="clip-list">{clips.map((clip, i) => {
      const scoreTooltip = Object.entries(SCORE_LABELS).map(([key, label]) => `${label}: ${(clip.scores as unknown as Record<string, number>)[key] ?? "—"}/5`).join(" · ");
      return <article key={`${clip.rank}-${clip.title}`} className={`clip-card ${active === i ? "active" : ""} ${selected[i] ? "selected" : ""}`} onClick={() => setActive(i)}><button className="check-btn" onClick={(e) => { e.stopPropagation(); setSelected(selected.map((v, index) => index === i ? !v : v)); }}><Icon name="check"/></button><div className="clip-main"><div className="clip-tags"><span>{(clip.topic || clip.pacing).toUpperCase()}</span><small>{timestamp(clip.start_second)} — {timestamp(clip.end_second)} · {timestamp(clip.estimated_duration).slice(3)}</small></div><h3>{clip.title}</h3><p className="clip-headline">{clip.headline}</p><p>{clip.reasoning}</p>{clip.warnings.length > 0 && <div className="clip-warnings">{clip.warnings.map((warning, index) => <span key={index} className="warning-chip">{warning}</span>)}</div>}<div className="micro-structure"><div className="beat hook"><i>HOOK</i><small>{timestamp(clip.hook.start_second)}–{timestamp(clip.hook.end_second)}</small><p>{clip.hook.summary}</p></div><div className="beat context"><i>CONTEXTO</i><small>{timestamp(clip.context.start_second)}–{timestamp(clip.context.end_second)}</small><p>{clip.context.summary}</p></div><div className="beat payoff"><i>PAYOFF</i><small>{timestamp(clip.payoff.start_second)}–{timestamp(clip.payoff.end_second)}</small><p>{clip.payoff.summary}</p></div></div></div><div className="score" title={scoreTooltip}><b>{Math.round((clip.scores.total / 35) * 100)}</b><small>EDITORIAL<br/>SCORE</small></div></article>;
    })}</div>
      ...
      <button className="btn secondary" disabled={detectingFaces || !sceneIndex} title={!sceneIndex ? "Detecte cenas antes de detectar rostos/planos" : undefined} onClick={onDetectFaces}><Icon name="film"/> {detectingFaces ? "Detectando rostos/planos..." : faceIndex ? "Redetectar rostos/planos" : "Detectar rostos/planos"}</button>{faceIndex && <span className="scene-index-summary">{faceIndex.frame_count} frame{faceIndex.frame_count === 1 ? "" : "s"} analisados · YuNet CPU</span>}</div>
      ...
      {detectingFaces && <div className="process-strip compact-process"><div className="process-meta"><span><i/> {faceMessage}</span><b>{Math.round(faceProgress)}%</b></div><div className="process-track"><i style={{width:`${faceProgress}%`}}/></div></div>}
      {faceError && <div className="error-strip" role="alert"><b>Falha na detecção de rostos/planos</b><p>{faceError}</p></div>}
      <FaceShotBadges faceIndex={faceIndex} sceneIndex={sceneIndex} start={activeClip.start_second} end={activeClip.end_second}/>
      <label className="jl-toggle"><input type="checkbox" checked={jlCutEnabled} disabled={planning} onChange={(event) => onJlCutEnabledChange(event.target.checked)}/> Habilitar J/L-cut (fronteiras de áudio/vídeo independentes)</label>
      {activePlan ? <><EditPlanTimeline clip={activeClip} plan={activePlan}/><button className="btn secondary plan-btn" disabled={planning} onClick={() => onRegeneratePlan(activeClip)}><Icon name="cut"/> {planning ? "Regerando..." : jlCutEnabled === activePlan.jl_settings.enabled ? "Regerar EDL" : jlCutEnabled ? "Regerar com J/L-cut" : "Regerar sem J/L-cut (cancelar)"}</button></> : <button className="btn secondary plan-btn" disabled={planning} onClick={() => void onPlan([activeClip])}><Icon name="cut"/> {planning ? "Resolvendo fronteiras..." : "Resolver EDL deste corte"}</button>}{planning && <div className="process-strip compact-process"><div className="process-meta"><span><i/> {planMessage}</span><b>{Math.round(planProgress)}%</b></div><div className="process-track"><i style={{width:`${planProgress}%`}}/></div></div>}{planError && <div className="error-strip" role="alert"><b>Falha no plano de edição</b><p>{planError}</p></div>}<div className="transcript-focus"><p>{activeWords.length ? activeWords.map((word, index) => <span className="spoken-word" key={`${word.start}-${index}`}>{word.word} </span>) : "Transcript indisponível para esta região."}</p><div className="boundary"><i/> {activePauses.length} PAUSAS · {Math.round((analysis?.overall_speech_ratio ?? 0) * 100)}% VOZ NO EPISÓDIO</div></div>
      {showFillers && <div className="fillers-panel">{analysis?.fillers === undefined ? <p className="fillers-empty">Análise sem fillers — reprocesse a análise para detectar hesitações.</p> : activeFillers.length === 0 ? <p className="fillers-empty">Nenhuma hesitação detectada neste corte.</p> : <><span className="fillers-count">{activeFillers.length} hesitaç{activeFillers.length === 1 ? "ão" : "ões"} detectada{activeFillers.length === 1 ? "" : "s"}</span><div className="fillers-list">{activeFillers.map((filler, index) => <span key={`${filler.start}-${index}`} className={`filler-chip ${filler.kind}`} title={`${filler.kind === "hesitation" ? "Hesitação" : "Repetição"} · confiança ${Math.round(filler.confidence * 100)}%`}>{filler.word} · {timestamp(filler.start)}</span>)}</div></>}</div>}
      {activePlan && <div className={`quality-report ${activePlan.quality.degraded ? "degraded" : "passed"}`}><div><Icon name={activePlan.quality.degraded ? "sliders" : "check"}/><b>{activePlan.quality.degraded ? "Plano preservado integralmente" : "Quality gate aprovado"}</b></div>{activePlan.quality.issues.length === 0 ? <p>Nenhum aviso de fronteira.</p> : activePlan.quality.issues.map((issue, index) => <p key={`${issue.code}-${index}`}><span>{issue.code}</span>{issue.snapped_from != null && issue.snapped_to != null ? ` ${issue.snapped_from.toFixed(3)}s → ${issue.snapped_to.toFixed(3)}s` : issue.word ? ` · ${issue.word}` : ""}</p>)}</div>}<div className="editor-notes"><div><span>Ritmo {activePlan ? "efetivo" : "sugerido"}</span><b>{activePlan?.profile ?? activeClip.pacing}</b></div><div><span>Densidade de fala do trecho</span><b>{activeSpeechRatio != null ? `${Math.round(activeSpeechRatio * 100)}%` : "—"}</b></div><div><span>Loudness análise</span><b>{analysis?.loudness.integrated_lufs ?? "—"} LUFS</b></div><div><span>True peak</span><b>{analysis?.loudness.true_peak_dbfs ?? "—"} dBFS</b></div></div></div></div>}
...
function PhonePreview({ platform, headline, headlineEnabled, captions, wireframe, frameUrl }: { platform: string; headline: string; headlineEnabled: boolean; captions: RenderSettings["captions"]; wireframe: boolean; frameUrl?: string }) {
  ...
  const captionStyle: CSSProperties = {
    ...
    fontFamily: captions.font_family,
    fontSize: `${Math.max(4, captions.font_size * 0.34)}px`,
    textShadow: captions.shadow ? `0 2px 6px ${captions.shadow_color}` : "none",
...
function LegacyApp() {
  ...
  const [jlCutEnabled, setJlCutEnabled] = useState(false);
  ...
  async function startSuggestion(brief: SuggestionBrief) {
    ...
    try {
      ...
      const provenance = finished.result?.provenance as SuggestionSelection["provenance"] | undefined;
      setSelection({ ...result, provenance }); setSuggestionProgress(100); setStep(3);
    } catch (error) {
  ...
  async function detectFaces() {
    const current = projectRef.current;
    if (!health || detectingFaces || !current?.sourceAssetId || !sceneIndexArtifactId) {
      setFaceError("Detecte cenas antes de detectar rostos/planos");
      return;
    }
    setDetectingFaces(true); setFaceError(null); setFaceProgress(0); setFaceMessage("Detecção de rostos na fila");
    try {
      const job = await api.startFaceIndex(current.projectId, current.sourceAssetId, sceneIndexArtifactId);
      setJobs((items) => [job, ...items]);
      const finished = await api.watchJob(job.id, (update) => {
        setFaceProgress(update.progress ?? 0);
        setFaceMessage(update.message || "Executando YuNet (onnxruntime CPU)");
        setJobs((items) => items.map((item) => item.id === update.id ? update : item));
      });
      if (finished.status !== "succeeded") throw new Error(finished.error || `Detecção de rostos terminou como ${finished.status}`);
  ...
  async function prepareEditPlans(clips: SuggestedClip[], options?: { force?: boolean }): Promise<boolean> {
    const current = projectRef.current;
    ...
    const pending = options?.force ? clips : clips.filter((clip) => !editPlans[clipKey(clip)]);
    ...
    try {
      for (let index = 0; index < pending.length; index += 1) {
        ...
        const job = await api.startEditPlan(current.projectId, current.transcriptArtifactId, current.analysisArtifactId, clip.start_second, clip.end_second, clip.pacing, sceneIndexArtifactId ?? undefined, jlCutEnabled);
        setJobs((items) => [job, ...items]);
        const finished = await api.watchJob(job.id, (update) => {
  ...
  function resetSourceArtifacts() {
    projectRef.current = null; setTranscript(null); setAnalysis(null); setSelection(null); setEditPlans({}); setEditPlanArtifactIds({}); setPreparedClipKeys([]); setRenderResults({}); setRenderStatuses({}); setRenderOverrides({}); setTranscriptionError(null); setSceneIndex(null); setSceneIndexArtifactId(null); setSceneError(null); setFaceIndex(null); setFaceIndexArtifactId(null); setFaceError(null); setIdentityIndex(null); setIdentityIndexArtifactId(null); setTargetIdentityId(""); setIdentityError(null); setJlCutEnabled(false);
  }

  async function regeneratePlan(clip: SuggestedClip) {
    setEditPlans((plans) => { const next = { ...plans }; delete next[clipKey(clip)]; return next; });
    setEditPlanArtifactIds((ids) => { const next = { ...ids }; delete next[clipKey(clip)]; return next; });
