...
import { api, type AnalysisDocument, type ApiJob, type EditPlanDocument, type RenderDocument, type SuggestedClip, type SuggestionBrief, type SuggestionSelection, type Telemetry, type TranscriptDocument, type TranscribeOverrides } from "./api";
...
function CurateStep({ clips, notes, transcript, analysis, plans, planning, planProgress, planMessage, planError, onPlan, onNext }: { clips: SuggestedClip[]; notes: string; transcript: TranscriptDocument | null; analysis: AnalysisDocument | null; plans: Record<string, EditPlanDocument>; planning: boolean; planProgress: number; planMessage: string; planError: string | null; onPlan: (clips: SuggestedClip[]) => Promise<boolean>; onNext: (clips: SuggestedClip[]) => Promise<void> }) {
...
function PhonePreview({ platform, headline, accent, captionSize, wireframe }: { platform: string; headline: string; accent: string; captionSize: number; wireframe: boolean }) {
  const description = "Uma conversa sobre fé e consciência";
  return <div className="phone-stage"><div className={`phone ${platform !== "clean" ? "with-overlay" : ""}`}><div className="video-noise"/><div className="speaker speaker-a"><span/></div><div className="speaker speaker-b"><span/></div><div className="headline-preview"><i>“</i>{headline.split("\n").map((line) => <b key={line}>{line}</b>)}</div><div className="caption-preview" style={{fontSize: `${captionSize}px`}}>A FÉ COMEÇA ONDE O <strong style={{backgroundColor: accent}}>CONTROLE</strong> TERMINA</div>{platform === "reels" && <ReelsOverlay description={description}/>}{platform === "tiktok" && <TikTokOverlay description={description}/>}{wireframe && <div className="safe-guides"><span>SAFE TITLE</span><i>SAFE CAPTION</i></div>}</div></div>;
}

function StudioStep({ onNext }: { onNext: () => void }) {
  const [tab, setTab] = useState("caption"); const [platform, setPlatform] = useState("reels"); const [wireframe, setWireframe] = useState(true); const [headline, setHeadline] = useState("A FÉ COMEÇA ONDE\nO CONTROLE TERMINA"); const [accent, setAccent] = useState("#19e6f2"); const [size, setSize] = useState(32); const [karaoke, setKaraoke] = useState(true); const [outline, setOutline] = useState(true);
  return <section className="step-content studio-wide enter"><div className="studio-toolbar"><div><span className="eyebrow">05 / VISUAL LAB <i>編集</i></span><h1>Estúdio visual</h1></div><div className="preview-switch"><button className={platform === "clean" ? "active" : ""} onClick={() => setPlatform("clean")}>Limpo</button><button className={platform === "tiktok" ? "active" : ""} onClick={() => setPlatform("tiktok")}>TikTok</button><button className={platform === "reels" ? "active" : ""} onClick={() => setPlatform("reels")}>Reels</button><button className={wireframe ? "safe active" : "safe"} onClick={() => setWireframe(!wireframe)}>Safe zones</button></div></div>
    <div className="studio-layout"><div className="inspector glass"><div className="inspector-tabs"><button className={tab === "caption" ? "active" : ""} onClick={() => setTab("caption")}>Legenda</button><button className={tab === "headline" ? "active" : ""} onClick={() => setTab("headline")}>Headline</button><button className={tab === "edit" ? "active" : ""} onClick={() => setTab("edit")}>Edição</button><button className={tab === "output" ? "active" : ""} onClick={() => setTab("output")}>Saída</button></div>
      {tab === "caption" && <div className="inspector-body"><Field label="Preset"><select defaultValue="Neat90"><option>Neat90</option><option>Karaoke Clean</option><option>Nova configuração...</option></select></Field><div className="split-fields"><Field label="Fonte"><select><option>Space Grotesk</option><option>Montserrat</option><option>Arial Black</option></select></Field><Field label="Peso"><select><option>700 · Bold</option><option>800 · Extra Bold</option><option>500 · Medium</option></select></Field></div><Range label="Tamanho" value={size} min={22} max={52} suffix="px" onChange={setSize}/><Range label="Palavras por caixa" value={5} min={2} max={9} onChange={() => {}}/><Field label="Cor de destaque"><div className="color-row"><input type="color" value={accent} onChange={(e) => setAccent(e.target.value)}/><input value={accent.toUpperCase()} onChange={(e) => setAccent(e.target.value)}/></div></Field><Toggle checked={karaoke} onChange={setKaraoke} label="Karaoke palavra por palavra"/><Toggle checked={outline} onChange={setOutline} label="Borda nos caracteres"/></div>}
      {tab === "headline" && <div className="inspector-body"><Field label="Texto da headline" hint="quebra manual"><textarea value={headline} onChange={(e) => setHeadline(e.target.value)} rows={4}/></Field><Field label="Template"><select><option>Editorial Quote</option><option>Signal Bar</option><option>Minimal Authority</option></select></Field><div className="split-fields"><Field label="Fonte"><select><option>Arial Black</option><option>Space Grotesk</option></select></Field><Field label="Faixas"><select><option>Uma por linha</option><option>Bloco único</option></select></Field></div><Range label="Duração" value={4} min={2} max={8} suffix="s" onChange={() => {}}/><Toggle checked={true} onChange={() => {}} label="Evitar rostos automaticamente"/><button className="btn secondary full"><Icon name="spark"/> Gerar novas sugestões</button></div>}
      {tab === "edit" && <div className="inspector-body"><Field label="Perfil de ritmo"><select><option>Auto · por semântica</option><option>Dinâmico</option><option>Equilibrado</option><option>Contemplativo</option></select></Field><Range label="Punch-in" value={6} min={0} max={12} suffix="%" onChange={() => {}}/><Range label="Crossfade de áudio" value={60} min={20} max={120} suffix="ms" onChange={() => {}}/><Toggle checked={true} onChange={() => {}} label="J/L-cut entre interlocutores"/><Toggle checked={true} onChange={() => {}} label="Inserir reação do entrevistador"/><Toggle checked={true} onChange={() => {}} label="Preservar pausas profundas"/></div>}
      {tab === "output" && <div className="inspector-body"><div className="split-fields"><Field label="Formato"><select><option>9:16 · Vertical</option><option>1:1 · Quadrado</option><option>16:9 · Horizontal</option></select></Field><Field label="Resolução"><select><option>1080 × 1920</option><option>2160 × 3840</option></select></Field></div><div className="split-fields"><Field label="Encoder"><select><option>H.264 NVENC</option><option>H.265 NVENC</option><option>libx264 CPU</option></select></Field><Field label="Qualidade"><select><option>Máxima · CQ 18</option><option>Alta · CQ 21</option><option>Compacta · CQ 25</option></select></Field></div><Toggle checked={true} onChange={() => {}} label="Gerar SRT separado"/><Toggle checked={true} onChange={() => {}} label="Normalizar áudio para -14 LUFS"/><Toggle checked={false} onChange={() => {}} label="Renderizar em 4K"/></div>}
    </div><PhonePreview platform={platform} headline={headline} accent={accent} captionSize={size} wireframe={wireframe}/><div className="quick-stack glass"><span className="eyebrow">LIVE PROPERTIES</span><div><span>Canvas</span><b>1080 × 1920</b></div><div><span>Safe zone</span><b>{platform === "tiktok" ? "TikTok" : "Instagram"}</b></div><div><span>Codec</span><b>NVENC H.264</b></div><div><span>Preset</span><b>Neat90</b></div><button className="btn secondary"><Icon name="save"/> Salvar preset</button></div></div>
    <div className="step-actions"><span>Alterações aplicadas em tempo real na prévia</span><button className="btn primary" onClick={onNext}>Revisar render <Icon name="chevron"/></button></div>
  </section>;
}
...
function RenderStep({ onStart, running, progress, clips, error, projectId, editPlanArtifactIds, results, statuses }: { onStart: () => void; running: boolean; progress: number; clips: SuggestedClip[]; error: string | null; projectId?: string; editPlanArtifactIds: Record<string, string>; results: Record<string, RenderResult>; statuses: Record<string, RenderItemStatus> }) {
  ...
  return <section className="step-content enter"><div className="page-heading"><div><span className="eyebrow">06 / PRODUCTION QUEUE <i>出力</i></span><h1>Pronto para produzir</h1><p>Confira o plano e dispare a fila. Cada corte é validado antes de ser entregue.</p></div><span className="phase-number">06</span></div><div className="render-grid"><div className="panel glass render-summary"><div className="summary-head"><span><Icon name="film"/></span><div><h3>Fila do projeto atual</h3><p>{clips.length} cortes · EDL persistida</p></div><b>{timestamp(duration)}</b></div>{clips.map((clip, i) => {
    const editPlanArtifactId = editPlanArtifactIds[clipKey(clip)]; const result = results[editPlanArtifactId]; const status = statuses[editPlanArtifactId] ?? { state: "queued", progress: 0 }; const quality = result?.document.quality;
    return <div className={`render-item ${status.state}`} key={clipKey(clip)}><span className="render-thumb">{String(i + 1).padStart(2, "0")}</span><div className="render-item-copy"><b>{clip.title}</b><small>{result ? `${timestamp(result.document.quality.actual_duration_seconds).slice(3)} · ${result.document.engine.width} × ${result.document.engine.height} · ${result.document.engine.effective_encoder}` : `${timestamp(clip.estimated_duration).slice(3)} · aguardando manifesto`}</small>{quality && <small className={quality.passed ? "quality-ok" : "quality-failed"}>{quality.passed ? "Quality gate aprovado" : `Quality gate reprovado: ${quality.issues.join(", ") || "sem detalhe"}`}</small>}{status.error && <small className="quality-failed">{status.error}</small>}</div><span className={`render-state ${status.state === "rendering" ? "working" : status.state}`}>{status.state === "rendering" ? `${Math.round(status.progress)}%` : status.state === "ready" ? (quality?.passed ? "PRONTO" : "REVISAR") : status.state === "failed" ? "FALHOU" : "EDL OK"}</span>{result && projectId && <div className="render-media"><video controls preload="metadata" src={api.renderMediaUrl(projectId, result.artifactId)} aria-label={`Prévia de ${clip.title}`}/><a className="btn secondary render-download" href={api.renderMediaUrl(projectId, result.artifactId)} download>Baixar MP4</a></div>}</div>;
  })}</div><div className="panel glass gates"><div className="panel-title"><span><Icon name={allPassed ? "check" : "sliders"}/></span><div><h3>Quality gate do render</h3><p>{completed.length ? `${completed.length} manifesto(s) persistido(s)` : "Aguardando outputs reais"}</p></div></div>{completed.length === 0 ? <div className="gate pending"><Icon name="clock"/><span>Nenhuma validação de mídia executada</span><b>PENDENTE</b></div> : completed.map(({ artifactId, document }) => <div className={`render-gate-group ${document.quality.passed ? "" : "failed"}`} key={artifactId}><div className="gate"><Icon name={document.quality.passed ? "check" : "sliders"}/><span>Áudio {document.quality.has_audio ? "presente" : "ausente"} · vídeo {document.quality.has_video ? "presente" : "ausente"}</span><b>{document.quality.passed ? "PASS" : "FAIL"}</b></div><small>{document.segment_count} segmento(s) · delta {document.quality.duration_delta_seconds.toFixed(3)}s · {document.engine.requested_encoder} → {document.engine.effective_encoder}</small>{document.quality.integrated_loudness_lufs != null && <small>{document.quality.integrated_loudness_lufs.toFixed(1)} LUFS · true peak {document.quality.true_peak_dbfs?.toFixed(1) ?? "—"} dBFS</small>}{document.quality.issues.map((issue) => <small className="quality-failed" key={issue}>{issue}</small>)}</div>)}</div></div>
    {running && <div className="render-progress glass"><div className="render-orbit"><span>{Math.round(progress)}%</span><i/></div><div><span className="eyebrow">RENDERING // NVENC</span><h3>Aplicando trims e crossfades da EDL</h3><p>{clips.length} cortes na fila persistente</p><div className="process-track"><i style={{width:`${progress}%`}}/></div></div></div>}
...
export default function App() {
  ...
  async function prepareEditPlans(clips: SuggestedClip[]): Promise<boolean> {
    ...
    try {
      for (let index = 0; index < pending.length; index += 1) {
        ...
        const job = await api.startEditPlan(current.projectId, current.transcriptArtifactId, current.analysisArtifactId, clip.start_second, clip.end_second, clip.pacing);
        setJobs((items) => [job, ...items]);
        const finished = await api.watchJob(job.id, (update) => {
  ...
  async function startRender() {
    ...
    try {
      for (let index = 0; index < artifactIds.length; index += 1) {
        ...
        const job = await api.startRender(current.projectId, editPlanArtifactId, "h264_nvenc");
        setJobs((items) => [job, ...items]);
        const finished = await api.watchJob(job.id, (update) => {
  ...
  return <div className="app-shell">
    ...
    <main className="workspace">
      {step === 0 && <SourceStep sourceMode={sourceMode} setSourceMode={setSourceMode} fileName={fileName} setFile={(f) => { setFile(f); projectRef.current = null; setTranscript(null); setAnalysis(null); setSelection(null); setEditPlans({}); setEditPlanArtifactIds({}); setPreparedClipKeys([]); setRenderResults({}); setRenderStatuses({}); setTranscriptionError(null); }} youtube={youtube} setYoutube={(v) => { setYoutube(v); projectRef.current = null; setTranscript(null); setAnalysis(null); setSelection(null); setEditPlans({}); setEditPlanArtifactIds({}); setPreparedClipKeys([]); setRenderResults({}); setRenderStatuses({}); }} onNext={() => setStep(1)}/>} 
      ...
      {step === 3 && <CurateStep clips={selection?.clips ?? []} notes={selection?.selection_notes ?? "Execute a seleção editorial na etapa anterior."} transcript={transcript} analysis={analysis} plans={editPlans} planning={planning} planProgress={planProgress} planMessage={planMessage} planError={planError} onPlan={prepareEditPlans} onNext={prepareAndOpenStudio}/>} 
      {step === 4 && <StudioStep onNext={() => setStep(5)}/>} 
      {step === 5 && <RenderStep onStart={startRender} running={rendering} progress={renderProgress} clips={(selection?.clips ?? []).filter((clip) => preparedClipKeys.includes(clipKey(clip)))} error={renderError} projectId={projectRef.current?.projectId} editPlanArtifactIds={editPlanArtifactIds} results={renderResults} statuses={renderStatuses}/>} 
    </main>
