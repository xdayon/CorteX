  ...
  return <>
    <div className="caption-preview-panel"><h3>Prévia ao vivo</h3><p>Teste de estilo sobre um frame do corte. A prévia curta confirma o movimento, o áudio e o enquadramento automático.</p>
      <div className="caption-preview" aria-label="Prévia da legenda em 1080 por 1920">
        <div className="caption-canvas">{image && <>{settings.framing.mode==='blurred_background' && <img className="preview-blur" src={image} alt=""/>}<img style={{objectPosition:`center ${Math.round((settings.framing.position_y ?? .5)*100)}%`}} className={settings.framing.mode==='vertical_crop'?'preview-cover':'preview-contain'} src={image} alt="Frame do corte selecionado"/></>}
