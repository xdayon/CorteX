...
export function JobProgress({job, history, progress, label, busy}: {
  job: ApiJob | null; history: JobActivity[]; progress: number; label: string; busy: boolean;
}) {
  ...
  return <section className="simple-progress job-progress" aria-label="Progresso da exportação">
    ...
    <details><summary>Exibir detalhes do processamento</summary>
      {job ? <><p><strong>{job.message || job.type}</strong> · {Math.round(job.progress || 0)}% desta tarefa</p>
        <dl><dt>Estado</dt><dd>{job.status}</dd><dt>Etapa</dt><dd>{job.stage || job.type}</dd>
