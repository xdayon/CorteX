...
> Estado atual em 2026-09-09: revisão de legenda por corte, prévia real e análise visual por intervalos.
> O processamento ate revisao e persistido e acompanhado ao reabrir. Exportacao
> ainda e agendada pelo navegador. Reações temporais são opt-in e exigem
> entrevistador explícito; incerteza de enquadramento preserva o quadro inteiro.
> As secoes datadas abaixo sao historico; README, ROADMAP e a atualizacao final
> deste documento descrevem o comportamento atual e substituem instrucoes antigas.
...
O legado foi removido em 2026-09-07; consulte o historico Git para comparacao.
Nao fazer reset/checkout destrutivo.
...
## Atualizacao 2026-09-07 - simplificacao do fluxo principal

- Backup antes de qualquer edicao: commit `c838544`, enviado e confirmado em
  `origin/codex/cortex-simplificacao`.
- Removidos `LegacyApp`, componentes/helpers/CSS exclusivos, preview de legenda
  simulado, botoes de estilo sem comportamento consistente e 25 arquivos legados
  de `mods/`, `reference/` e `apply.sh`. Arquivos ignorados e midia local preservados.
- Motores standalone, presets da API, Remotion, boundaries, caches, provenance e
  quality gates mantidos. O frontend expoe somente as tres telas atuais.
- `PUT /projects/{project_id}/runs/{run_id}/identity` agora exige
  `interviewer_identity_id`. O WorkflowRun persiste a escolha explicita;
  `subject_identity_id` antigo continua legivel sem virar entrevistador.
  Fonte, projeto, hashes e dependencias dos seis indices sao validados antes da
  escrita. A interface so muda a escolha confirmada apos sucesso da API.
- Reabertura acompanha jobs queued/running ate revisao e mostra falhas/cancelamento.
  Troca de episodio e unmount cancelam polling/SSE e ignoram respostas antigas.
  Falhas opcionais de camera permanecem visiveis junto dos resultados normais.
- Exportacao continua sequencial no navegador; selecao, settings e lista de
  exports nao sao restaurados. A fila duravel continua pendente no ROADMAP.
- Verificacao: suite backend completa 328 passed / 3 skipped; verificacao final
  de workflow + project-status 47 passed; Vitest/React Testing Library 17 passed;
  builds web e Remotion, Ruff nos arquivos Python alterados e diff-check limpos.
  Os tres skips sao E2E Remotion/browser opt-in; GPU/NVENC e episodio real nao
  foram revalidados nesta alteracao.
- Dependencias de teste frontend fixadas no lockfile; dois transitivos existentes
  (PostCSS/nanoid) atualizados dentro dos ranges permitidos. npm audit: 0 findings.


## Atualização 2026-09-08 — reenquadramento do master único

- Interface inicia no YouTube e oferece `speaker_auto` como composição padrão.
  Antes de renderizar, revalida os jobs de análise (cache por hash); não depende de
  escolher entrevistador para enquadrar quem aparece no vídeo.
- Corrigido o agrupamento global que atribuía `person` aos dois rostos da câmera
  aberta no episódio real. `assign_scene_tracks` separa slots por cena e descarta
  colisões; `FACE_ALGORITHM_VERSION=3.0.0` força renovar os índices dependentes.
- `render/auto_framing.py` resolve composição por plano/turno visual sustentado,
  segura breves oscilações de confiança, preserva ouvinte natural e usa quadro
  inteiro desfocado em ambiguidade. Não inventa câmera ausente no master.
- `scene_overrides` permite corrigir esquerda/direita/ambos. Punch-in opcional
  limita geometria e resolução; render v14 registra inputs, decisões, motivos,
  zoom efetivo e warnings. Áudio continua usando exclusivamente a EDL primária.
- Reações reaproveitadas de outro instante são opt-in e ainda exigem identidade
  explícita do entrevistador. Mudança do índice exige nova confirmação.
- Remotion usa no máximo dois workers de frames, adequados à memória do notebook.
- Evidência e comandos: `docs/evidence/2026-09-08-auto-framing.md`.
- Próximo gate: revisão humana do vídeo de teste e de estúdios diferentes. Fila
  de exportação continua agendada pela aba; seleção/settings não são restaurados
  ao reabrir. Nenhum pacote novo de ML/CUDA ou provider editorial foi instalado.

Validação deste gate: 351 testes backend passaram com Remotion/browser opt-in,
21 testes frontend, builds Web/Remotion, Ruff e diff-check passaram. O smoke GPU
confirmou NVENC e CUDA int8; o comparativo real preservou áudio idêntico.
Demonstração final com karaoke Remotion + zoom: `render-369b1b40d1fb4407`,
34,33 s, NVENC, publish_ready=true e cache confirmado; detalhes no relatório.


## Correção após uso em episódio longo — 2026-09-08

- Exportação inicia em quadro inteiro desfocado, sem análise visual obrigatória.
- Automático permanece explícito: ainda analisa o episódio completo. A validação
  anterior em 34 s não comprova desempenho adequado em podcasts longos.
- Progresso visual agora exibe percentual do job da etapa atual; anteriormente
  a UI mostrava o percentual do render (0%) durante toda a preparação visual.
- Mensagem facial informa amostras processadas/total, distinguindo posição no
  episódio de quantidade de frames. Sem mudança de modelo ou stack.
- Pendente: restringir análise automática aos cortes selecionados, com cache e
  cobertura temporal validados. Quadro inteiro é mitigação, não solução do close.

## Atualização 2026-09-09 — legenda e novo episódio

Prévia ao vivo compartilha CaptionLayer com Remotion; fonte, negrito, caixa,
tamanho, posição vertical, blocos e efeitos chegam ao export. Novo padrão 56 px.
Começar outro episódio guarda draft/histórico local e libera fonte nova sem
purge. Retomar restaura cortes/settings. Evidência e limites em
`docs/evidence/2026-09-09-caption-editor.md`. Reenquadramento automático restrito
aos cortes e fila durável continuam pendentes; não foram alterados neste gate.

## Atualização 2026-09-09 — revisão por corte

Editor de palavras salva correções por corte sem alterar timestamps/áudio/fonte;
preview real de ~10 s separado do lote. Análise visual usa intervalos selecionados
+3 s, cache por escopo e tempos absolutos. Primeira análise ainda CPU; não há
aceleração de seek no render. Evidência: `docs/evidence/2026-09-09-clip-review.md`.
353 testes backend com E2E passaram; 28 frontend e builds. Fila durável pendente.

## Atualização 2026-09-09 — download YouTube 403

Extra youtube agora instala yt-dlp[default]>=2026.8.19; EJS e Node explícito.
Vídeo wk7PY31TQBc baixado integralmente pela API e persistido. Cache não aceita
faixas intermediárias como vídeo final. 17 testes ingest/config passaram.
Evidência: docs/evidence/2026-09-09-youtube-403.md.
