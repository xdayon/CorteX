...
- [x] processamento ate a revisao persistido no backend e acompanhado novamente ao reabrir;
- [x] entrevistador escolhido explicitamente e reacoes seguras de 0,7–1,6s como enriquecimento opcional com falhas visiveis;

- [x] remover frontend legado inacessivel, CSS/helpers exclusivos e controles simulados;

Os itens historicos de Studio/Curadoria abaixo registram capacidades desenvolvidas.
Os motores e endpoints continuam disponiveis; o frontend atual expoe somente as
opcoes do fluxo de tres telas descrito no README.

## Enquadramento do master de podcast — 2026-09-08

- [x] `speaker_auto` integrado à exportação: análise de câmeras local antes do render,
  recorte estável por plano/turno visual sustentado e preservação do ouvinte visível.
- [x] slots de rosto separados por cena, sem atribuir o mesmo track a dois rostos
  simultâneos; algoritmo v3 invalida caches anteriores.
- [x] incerteza preserva o quadro inteiro com fundo desfocado, registrada no manifesto
  e no relatório de publicação; falha da análise não vira recorte central silencioso.
- [x] correção manual esquerda/direita/quadro inteiro por cena e zoom alternado 1,15×,
  com limites de rosto/resolução; escolhas e geometria efetiva persistidas no render v14.
- [x] reutilização temporal de reações desligada por padrão, com opção explícita;
  enquadramento automático não exige identificar o entrevistador.
- [x] regressões FFmpeg verificam pixels, continuidade de áudio, cache e cadeia de fonte;
  comparação NVENC em trecho real de 34,33 s aprovada (evidência abaixo).
- [ ] ampliar a avaliação humana para diferentes estúdios e episódios completos.

Evidência: `docs/evidence/2026-09-08-auto-framing.md`. Este gate não conclui a fila
persistente de exportação nem adiciona diarização acústica, fontes ISO ou tracking contínuo.
...
- [ ] fila de exportacao duravel de ate 25 cortes e retomada da selecao/settings/resultados;
  jobs enviados sao persistidos, mas o navegador ainda agenda cada proximo corte;
- [ ] relatorio de QA e comparacao antes/depois (Gate 7 e runners prontos;
  relatorio consolidado Gate 9 depende da matriz real);
- [x] remocao definitiva do legado PodCLI; runtime e testes usam os motores standalone.
...
## Editor de legenda — 2026-09-09

- [x] prévia animada compartilhando componente de legenda com render real;
- [x] fonte, peso, caixa e posição configuráveis, além de cores e blocos;
- [x] começar outro episódio sem apagar artifacts, com retomada via histórico local;
- [x] edição da transcrição por corte e preview curto do resultado final antes do lote (gate de revisão abaixo).

Evidência: `docs/evidence/2026-09-09-caption-editor.md`.

## Revisão por corte — 2026-09-09

- [x] corrigir palavras por corte sem modificar a transcrição original ou áudio;
- [x] prévia curta real antes do lote, usando settings/correções da exportação;
- [x] restringir cenas, faces, speaker e qualidade visual aos intervalos escolhidos;
- [ ] reduzir custo de decodificação do render preservando áudio e timestamps.

Evidência: `docs/evidence/2026-09-09-clip-review.md`. Primeira análise ainda é CPU;
preview tem resolução final e não é instantâneo. Não houve migração de stack.

- [x] importação YouTube com yt-dlp/EJS e runtime Node habilitado, validada em
  download integral do link que retornava403 (evidência2026-09-09-youtube-403).
