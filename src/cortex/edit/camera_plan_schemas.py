...
class CameraEditShot(BaseModel):
    ...
    visual_origin: VisualOrigin = "primary_in_place"
    ...
    def validate_synchronized_sources(self) -> "CameraEditShot":
        ...
        if self.visual_origin == "primary_in_place":
            if self.source_start_us != self.audio_source_start_us or self.source_end_us != self.audio_source_end_us:
                raise ValueError("primary visual must match the editorial audio interval")
            if self.sync_offset_us != 0:
                raise ValueError("primary visual cannot have sync offset")
        elif self.visual_origin == "reaction_reuse":
            if self.video_source_asset_id != self.audio_source_asset_id or self.sync_offset_us != 0:
                raise ValueError("reaction reuse must borrow video from the same master source")
